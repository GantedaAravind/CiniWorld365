...
function Movies() {
  ...
  const [page,setPage] = useState<PayeType>({"now_playing":1,"popular":1,"top_rated":1,"upcoming":1});
  const handleLoadMore = ()=>{
    const currentCategory = category.filter(item => item.name === filter);
    if(currentCategory){
      console.log(currentCategory[0].path);
      setPage((prev)=>{
        return {...prev, [currentCategory[0].path]:prev[currentCategory[0].path]+1};
      })
  ...
  const fetchMovies = async (path: string, page: number) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/${path}?language=en-US&page=${page}`,
        options
      );
